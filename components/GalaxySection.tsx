...
const GalaxySection: React.FC = () => {
  ...
  const [activeVideoCourse, setActiveVideoCourse] = useState<{ id: string, takeaway: string, color: string } | null>(null);
  const { getCourseProgress } = useProgressStore();

  return (
    <section className="relative py-20 md:py-32 overflow-hidden px-4" id="galaxy-section">
      ...
      <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 relative z-10">
              ...
              >
                ...
                <div className="flex items-center justify-between mb-6 md:mb-8">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    <button 
                      onClick={(e) => {
                        e.preventDefault();
                        setActiveVideoCourse({ id: course.id, takeaway: course.takeaway, color: course.color });
                      }}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-blue-600/10 border border-blue-500/20 text-blue-500 hover:bg-blue-600 hover:text-white transition-all text-[9px] font-black uppercase group/trailer shadow-lg"
                    >
                      <Play className="w-3 h-3 fill-current group-hover/trailer:animate-pulse" />
                      AI Trailer
                    </button>
                  </div>
                </div>
                ...
                >
                  <div className="flex justify-between items-start mb-3 md:mb-4">
                    <h3 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-blue-600 group-hover:to-purple-600 dark:group-hover:from-white dark:group-hover:to-blue-400 transition-all duration-300">
                      {course.title}
                    </h3>
                    {progress === 100 && <CheckCircle className="w-5 h-5 text-emerald-500" />}
                  </div>
                  ...
                  {/* Planet Progress Indicator */}
                  <div className="mb-6 space-y-2">
                    <div className="flex justify-between text-[9px] font-black text-gray-400 uppercase tracking-widest">
                      <span>Neural Mastery</span>
                      <span className={progress > 0 ? 'text-blue-400' : ''}>{progress}%</span>
                    </div>
        ...
        <AnimatePresence>
          {activeVideoCourse && (
            <CourseVideoPreview 
              courseId={activeVideoCourse.id}
              takeaway={activeVideoCourse.takeaway}
              color={activeVideoCourse.color}
