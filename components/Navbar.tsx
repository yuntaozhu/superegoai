...
const Navbar: React.FC = () => {
  ...
  const navBorderClass = currentCourse 
  ...
  return (
    <nav className="fixed w-full z-50 bg-brand-dark/90 backdrop-blur-md transition-all duration-500" style={navStyle}>
      {/* Gradient Border Line */}
      {currentCourse && (
         <div className={`absolute bottom-0 left-0 w-full h-[1px] bg-gradient-to-r ${currentCourse.color} opacity-80`} />
      )}
      {!currentCourse && (
        ...
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center">
            <Link to="/" className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center bg-gradient-to-tr ${currentCourse ? currentCourse.color : 'from-blue-500 to-purple-600'}`}>
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-center space-x-4">
              ...
              <Link
                to="/consulting"
                className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                  isActive('/consulting') ? 'bg-brand-surface text-white' : 'text-gray-300 hover:text-white hover:bg-white/10'
                }`}
              >
                {t('nav.consulting')}
              </Link>
