...
const ResearchPlanet: React.FC = () => {
  ...
  const modules = [
    ...
    },
    {
      ...
      outcome: '具有竞争力的多 Agent 系统与科研数据集',
    ...
    },
    {
      ...
      outcome: '个性化 AI ExtBrain 系统与愿景演示',
  ...
  return (
    <PlanetLayout course={course}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <span className="text-indigo-400 font-mono text-xs uppercase tracking-[0.5em]">ExtBrain_Research_OS</span>
            </div>
            ...
            <p className="text-xl text-gray-300 leading-relaxed font-light">
              不仅仅是编程课，而是构建“私人科研中台”。通过 <span className="text-indigo-400 font-bold">建模 (Approach)</span> 真实世界，辅助高阶学术研究，在高级竞赛中取得跨代优势。
            </p>
          </motion.div>

          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {coreAbilities.map((a, i) => (
              <div key={i} className={`p-6 bg-white/5 border border-white/10 rounded-[32px] text-center hover:bg-white/10 transition-colors ${i === 4 ? 'col-span-2' : ''}`}>
                 <div className="text-3xl mb-4">{a.icon}</div>
                 <h4 className="text-white font-bold text-sm mb-2">{a.name}</h4>
                 <p className="text-gray-500 text-[9px] uppercase font-mono">{a.desc.substring(0, 30)}...</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-20">
          {modules.map((m, idx) => (
            <motion.div 
              ...
              viewport={{ once: true }}
              className="group relative bg-brand-surface border border-white/10 rounded-[56px] overflow-hidden backdrop-blur-xl hover:border-indigo-500/20 transition-all duration-500"
            >
              ...
              <div className="p-12 md:p-20 flex flex-col lg:flex-row gap-12">
                 <div className="lg:w-1/3 space-y-8">
                    <div className="flex items-center gap-4">
                       ...
                       <div className="text-xs font-mono text-gray-600">LEVEL_0{idx + 1}</div>
                    </div>
                    <h3 className="text-3xl font-black text-white uppercase tracking-tighter leading-tight">{m.title}</h3>
                    <p className="text-gray-400 text-sm italic">{m.goal}</p>
                 ...
                 <div className="lg:w-2/3 flex flex-col gap-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      {m.content.map((item, i) => (
                        <div key={i} className="p-8 bg-black/40 rounded-[32px] border border-white/5 flex gap-4 items-start group-hover:bg-black/60 transition-colors">
                           <div className="w-8 h-8 rounded-full bg-indigo-500/10 flex items-center justify-center text-indigo-400 font-mono text-[10px] flex-shrink-0">
                             {i + 1}
                           </div>
                    ...
                    <div className={`p-8 bg-gradient-to-br ${m.gradient} rounded-3xl text-center relative overflow-hidden group-hover:scale-[1.01] transition-transform duration-500 shadow-2xl`}>
                       <span className="text-[10px] font-mono text-white/60 uppercase tracking-widest">Stage Deliverable // 阶段产出</span>
                       <div className="text-white font-black mt-2 text-xl tracking-tight uppercase">{m.outcome}</div>
                       <div className="absolute top-0 right-0 p-4 opacity-10 text-6xl">{m.icon}</div>
                    </div>
                 </div>
              </div>
            </motion.div>
      ...
      <section className="mt-40 mb-20">
        <div className="max-w-4xl mx-auto p-20 bg-indigo-900/10 border border-indigo-500/20 rounded-[64px] backdrop-blur-xl relative overflow-hidden text-center">
          <div className="relative z-10">
            <h3 className="text-4xl font-black text-white mb-8 uppercase tracking-tighter">费曼学习法：深度进化的灵魂</h3>
            <p className="text-gray-300 text-lg leading-relaxed font-light mb-12">
              不仅是完成代码，还需通过 <span className="text-indigo-400 font-bold">“费曼时刻”</span> 总结 AI 逻辑。如果你不能向一个六岁小孩解释清楚，说明你还没真正掌握这个科学模型。
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
              <div className="p-8 bg-black/40 rounded-3xl border border-white/5">
                <h4 className="text-indigo-300 font-bold mb-3">费曼时刻 (Review)</h4>
                <p className="text-gray-500 text-xs italic">每节课后，轮流用最通俗语言总结科研 Agent 的协作逻辑。</p>
              </div>
              <div className="p-8 bg-black/40 rounded-3xl border border-white/5">
                <h4 className="text-indigo-300 font-bold mb-3">费曼挑战 (Create)</h4>
                <p className="text-gray-500 text-xs italic">产出“愿景与演示组合”，展示 ExtBrain 在特定领域的实战价值。</p>
              </div>
            </div>
          </div>
